...
const Logo = () => (
  <div className="flex flex-col px-4">
    <div className="flex items-center space-x-3">
      ...
      <span className="text-2xl font-bold text-brand-green-dark tracking-tight">HealWise</span>
    </div>
    <span className="text-[10px] text-gray-400 mt-1.5 ml-1 tracking-wider font-medium uppercase">a Foster Wealth Ventures creation</span>
  </div>
);

const NavItem: React.FC<{ icon: React.ReactElement; label: string; isActive: boolean; onClick: () => void; isAction?: boolean; }> = ({ icon, label, isActive, onClick, isAction = false }) => (
  <button
    onClick={onClick}
...
const Sidebar: React.FC<{ activeView: string; setActiveView: (view: string) => void }> = ({ activeView, setActiveView }) => {
  const navItems = [
    ...
    { id: 'planner', label: 'My Planner', icon: <PlannerIcon /> },
    { id: 'shop', label: 'HealWise Market', icon: <ShopIcon /> },
  ];

  return (
    <aside className="w-64 bg-white/80 backdrop-blur-sm border-r border-gray-200/80 flex-shrink-0 flex-col hidden lg:flex">
      <div className="h-24 flex items-center justify-start pl-4 border-b border-gray-200/80">
         <Logo />
      </div>
      ...
      <div className="p-6 border-t border-gray-200/80">
          <h4 className="font-semibold text-brand-green-dark">Food-First, Faith-Friendly</h4>
          <p className="text-sm text-gray-500 mt-1">Your journey to holistic wellness starts here.</p>
      </div>
    </aside>
  );
};
